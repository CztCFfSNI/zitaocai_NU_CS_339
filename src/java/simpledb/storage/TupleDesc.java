...
public class TupleDesc implements Serializable {
    ...
    public TupleDesc(Type[] typeAr, String[] fieldAr) {
        // some code goes here
        if (typeAr.length != fieldAr.length) throw new IllegalArgumentException("The lengths of typeAr and fieldAr should be the same.");
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        ...
        for (int i = 0; i < items.length; i++) 
            if (items[i].fieldName != "" && items[i].fieldName.equals(name)) return i;
    ...
    public int getSize() {
        // some code goes here
        int size = 0;
        for (TDItem item : items) size += item.fieldType.getLen();
        return size;
    }
